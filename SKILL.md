...
### v1.2.0 — 报告视觉重设计 (2026-08-14)

基于 /idesign 对报告模板进行视觉重设计，参考现代 SaaS 浅色风格。

**视觉改动**：
- 新基调：浅蓝灰 #F4F7FA 背景 + 白色卡片 + 亮蓝 #0456FC 强调色
- 三灯分级改为克制呈现：色点 + 浅底徽章 + 顶部小图标，移除左侧 4px 彩色边条
- 侧边栏升级：亮蓝 logo + 图例色点导航 + 选中浅蓝 pill
- 图标全面替换为内联 SVG（check/alert/trash/导航）
- 磁盘状态条细化为 12px 圆角分段条，移除加载 width 动画
- 进度条改 transform scaleX 动画（避免 layout thrash）
- 命令块深蓝黑底 + 亮蓝复制按钮
- 通过 idesign 机械检测（0 findings）

**报告模板**：report_template.html 视觉层重写，功能/JS 逻辑 100% 保留。
